        ...
        <section id="certificates" className="pt-2 py-5" data-label="certificates">
            <div className="p-6 md:p-10">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4 md:gap-5 lg:gap-6">
                    ...
                    const hoverEffect = "transform transition-all duration-300 hover:scale-105 hover:shadow-xl";
                    ...
                    if (cert.image) {
                        return (
                            <div
                                key={cert.id}
                                className={`group relative flex items-center justify-center cursor-pointer ${hoverEffect}`}
                            ...
                            >
                                <img
                                    src={cert.image}
                                    alt={cert.title}
                                    className="w-full h-auto object-contain filter drop-shadow-lg group-hover:drop-shadow-2xl transition-all rounded-2xl"
                                />
                            </div>
                        );
                    }
                    ...
                    if (isBlue) {
                        containerClasses = "bg-blue-500/[0.05] border-blue-500/20 hover:bg-blue-500/[0.12] hover:border-blue-500/40";
                        iconBg = "bg-blue-500/20 text-blue-400";
                        titleHover = "group-hover:text-blue-200";
                        yearBadge = "bg-blue-500/20 text-blue-300 border border-blue-500/30";
                    }
                    if (isGreen) {
                        containerClasses = "bg-emerald-500/[0.05] border-emerald-500/20 hover:bg-emerald-500/[0.12] hover:border-emerald-500/40";
                        iconBg = "bg-emerald-500/20 text-emerald-400";
                        titleHover = "group-hover:text-emerald-200";
                        yearBadge = "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30";
                    }
                    if (isYellow) {
                        containerClasses = "bg-yellow-500/[0.05] border-yellow-500/20 hover:bg-yellow-500/[0.12] hover:border-yellow-500/40";
                    ...
                    if (isViolet) {
                        containerClasses = "bg-violet-500/[0.05] border-violet-500/20 hover:bg-violet-500/[0.12] hover:border-violet-500/40";
                        iconBg = "bg-violet-500/20 text-violet-300";
                        titleHover = "group-hover:text-violet-200";
                        yearBadge = "bg-violet-500/20 text-violet-300 border border-violet-500/30";
                    }
                    if (isRed) {
                        containerClasses = "bg-red-500/[0.05] border-red-500/20 hover:bg-red-500/[0.12] hover:border-red-500/40";
                    ...
                    return (
                        <div
                            key={cert.id}
                            className={`group relative p-3 sm:p-4 rounded-xl border cursor-pointer 
                            aspect-square flex flex-col justify-between ${containerClasses} ${hoverEffect}`}
                            onClick={() => handleOpenCertificate(cert.path)}
                        >
                            <div className="flex items-start justify-between">
                                ...
                                <span className={`text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.5 rounded ${yearBadge}`}>
                                    {cert.year}
                                </span>
                            </div>
